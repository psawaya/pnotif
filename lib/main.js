...
var Client = {
    ...
    init: function init() {
        ...
        Engine.loadSubscriptions(function() {
            if (Engine.getQueueObj()) {
                haveLoggedIn = true;
                Client.connectProtocol(function() {});
            }
        });
    },
    ...
    connectProtocol: function connectProtocol(successCallback) {
        if (Client.USE_SSE)
            Client.connectSSE(successCallback);
        else
            Client.connectAMQP(successCallback);
    },
    
    connectSSE: function connectSSE(successCallback) {
        var queueObj = Engine.getQueueObj();

        var wm = Cc["@mozilla.org/appshell/window-mediator;1"]
                    .getService(Ci.nsIWindowMediator);
        var win = wm.getMostRecentWindow("navigator:browser");
        var SSE = new win.EventSource('http://localhost:8888/feed/' + queueObj.queueName);
        var thisClient = this;
        SSE.addEventListener('message', function(e) {
    		thisClient.onConsumeNotification(e.data);
    	});
    	if (successCallback)
            successCallback();   
    },
    ...
    createUserQueue: function createUserQueue(successCallback) {
        ...
        res.post("", function(error, result) {
            ...
            Client.connectProtocol(successCallback);
        });
    },
